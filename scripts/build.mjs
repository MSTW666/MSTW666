...
const OFFLINE_OK = new Set(["hero", "about", "fortune", "photoband", "display", "tiles", "quote", "sections", "contact"]) // needs no API data
...
try {
  ...
  cfg.__lastKnownGood = {
    cards: Array.isArray(prev?.cards) ? prev.cards : null,
    scopeRepos: Array.isArray(prev?.scopeRepos) ? prev.scopeRepos : null,
  }
} catch { /* no state yet — first build, or the file was removed on purpose */ }

let ctx
if (only && [...only].every(id => OFFLINE_OK.has(id))) {
  ctx = { now: new Date() }
...
if (!only || only.has("contact")) {
  const path = resolve(ROOT, "README.md")
  const src = await readFile(path, "utf8")
  const next = src.replace(/(<!-- CONTACT_START -->)[\s\S]*?(<!-- CONTACT_END -->)/,
    (_, a, b) => a + "\n" + contactLinks(cfg.contact) + "\n" + b)
...
//
// An authoritative empty pin list must clear the previous cards and state.
if (Array.isArray(ctx?.work?.picked) && (!only || only.has("work"))) {
