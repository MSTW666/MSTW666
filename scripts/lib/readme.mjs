...
/** Contact href and enabled state share the configuration used to draw buttons. */
export const contactLinks = (items) => items.filter(c => c.enabled).map(c => {
  if (!/^[a-z0-9-]+$/.test(c.key)) throw new Error('Invalid contact key')
  if (!/^(https?:\/\/|mailto:|assets\/)/.test(c.href)) throw new Error('Invalid contact URL')
  return '<a href="' + escapeAttr(c.href) + '"><picture><img alt="' + escapeAttr(c.label) +
    '" title="' + escapeAttr(c.label) + '" src="assets/generated/btn-' + c.key + '.svg"></picture></a>'
}).join('\n')
